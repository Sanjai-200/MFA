...
async function loadActivity(){
  ...
  try{
    ...
    if(!d.logs.length){tb.innerHTML=`<tr><td colspan="4" style="text-align:center;color:var(--muted);padding:20px">No activity yet</td></tr>`;return;}
...
async function loadSecurity(){
  try{
    ...
    document.getElementById("sec-device").textContent=last?last.device||"—":"—";
    document.getElementById("sec-location").textContent=last?last.location||"—":"—";
  }catch(e){toast(e.message,"error");}
}
